...
//			 "@ATTRIBUTE id_new {YES, NO, NA}\n" +
	...
	private static Logger logger = Logger.getLogger(ConversionToArffAnaphore.class);
	...
	public ConversionToArffAnaphore(Corpus corpus){
	 ...
	 */
	private ConversionToArffAnaphore(int positif, int negatif, ParamToArff param, String outputPath, int split){
	...
	public ConversionToArffAnaphore(Corpus corpus, int positif, int negatif, ParamToArff param, String outputPath, int split){
		this(positif, negatif, param, outputPath, split );
		this.corpusList.add( corpus );
	}

	public ConversionToArffAnaphore(List<Corpus> corpusList, int positif, int negatif, ParamToArff param,  String outputPath, int split){
		this(positif, negatif, param, outputPath, split );
		...
		for(Corpus corpus : corpusList){
			...
			for(Annotation annotation : corpus.getAnnotation()){
				...
				for( Relation relation : annotation.getRelation() ){
					...
					else if(this.param.equals( ParamToArff.ANAPHORE )){
					}
					...
					try {
						...
						for(int p = 0; p < this.positiveRelationSelected.size(); p++){
							...
							if(string_type_relation.equals("ANAPHORE")){
								...
								String line = this.makeRelation(this.positiveRelationSelected.get( relationArray[ p ] ), relationArray[ p ] );
								writer.println( line + "NOT_ANAPHORE" );		
...
//								relationArray2[ n ].getPreElement( this.negativeRelationSelected.get( relationArray2[ n ] ) ).getIdMention();
								...
								writer.println( line2 + "NOT_ANAPHORE" );		
