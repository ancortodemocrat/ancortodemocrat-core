...
public class ConversionToArffAssocPronom implements Runnable{
...
//			 "@ATTRIBUTE id_new {YES, NO, NA}\n" +
			 ...
			 "@ATTRIBUTE class {ASSOC_PRONOM, NOT_ASSOC_PRONOM}\n" +
			 "@DATA";


	private static Logger logger = Logger.getLogger(ConversionToArffAssocPronom.class);
	...
	public static int countAssocPronomRelation = 0;
	public static int countNotAssocPronomRelation = 0;
	...
	public ConversionToArffAssocPronom(Corpus corpus){
	 ...
	 * @param param Paramètre indiquant si on garde ou non les assocpronomiatives
	 * @param outputPath chemin de sortie du fichier arff
	 */
	private ConversionToArffAssocPronom(int positif, int negatif, ParamToArff param, String outputPath, int split){
	...
	public ConversionToArffAssocPronom(Corpus corpus, int positif, int negatif, ParamToArff param, String outputPath, int split){
		this(positif, negatif, param, outputPath, split );
		this.corpusList.add( corpus );
	}

	public ConversionToArffAssocPronom(List<Corpus> corpusList, int positif, int negatif, ParamToArff param,  String outputPath, int split){
		this(positif, negatif, param, outputPath, split );
			...
			for(Annotation annotation : corpus.getAnnotation()){
				...
				for( Relation relation : annotation.getRelation() ){
					if(this.param.equals( ParamToArff.NO_ASSOC )){
						//test si la relation est une assocpronomiative ou non
						//si c'est le cas, next
						if(relation.getCharacterisation().getType().getValue().toLowerCase().contains( "ASSOCPRONOM" ) ){
							continue;
						}
					}
					else if(this.param.equals( ParamToArff.ASSOCPRONOM )){
					}
				...
				for(int f = 1; f < split + 1; f++){
					try {
						...
						for( int l = start; l < end; l++){
							...
							if(string_type_relation.equals("ASSOC_PRONOM")){
								countAssocPronomRelation++;
								...
								writer.println( line + "ASSOC_PRONOM" );
						}
						
							else{
								countNotAssocPronomRelation++;
								...
								writer.println( line2 + "NOT_ASSOC_PRONOM" );		
							}
						}
				...
				try {
					...
					for(int p = 0; p < this.positiveRelationSelected.size(); p++){
						...
						if(string_type_relation.equals("ASSOC_PRONOM")){
							...
							String line = this.makeRelation(this.positiveRelationSelected.get( relationArray[ p ] ), relationArray[ p ] );
							writer.println( line + "NOT_ASSOC_PRONOM" );		
...
//						relationArray2[ n ].getPreElement( this.negativeRelationSelected.get( relationArray2[ n ] ) ).getIdMention();
	...
	public void run() {
		...
		logger.info("ASSOC_PRONOM: " + countAssocPronomRelation );
		logger.info("NOT-ASSOC_PRONOM: " + countNotAssocPronomRelation );
